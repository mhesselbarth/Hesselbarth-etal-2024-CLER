...
<!-- The strength of landscape metrics is that they are comparable easy to apply and communicate [@Lausch2015UnderstandingQuantifying], however, they are also critiqued for not being able to realistically represent continuous features of the natural word [@Frazier2017LandscapeMetrics]. -->
The strength of landscape metrics is that they are comparable easy to apply and communicate [@Lausch2015UnderstandingQuantifying], however, major limitations have been identified regarding their ability to quantify the spatial structure of landscapes as well as their sensitivity to both spatial scale and thematic resolution [@Gustafson1998QuantifyingLandscape; @Li2004UseMisuse; @Dramstad2009SpatialMetrics; @Kupfer2012LandscapeEcology; @Lausch2015UnderstandingQuantifying; @Frazier2017LandscapeMetrics].

Metrics can be calculated on patch-level (describing each patch), class-level (describing all patches of the same class), or landscape level (describing the entire landscape) and are able to quantify characteristics related to e.g., patch areas and edge lengths, patch shapes, or class aggregation and diversity [@Hesselbarth2019LandscapemetricsOpensource].

<!-- MB: I think we can put the part below within the "landscape metrics" section, otherwise we could add a subsection or even subsubsection entitled "Fundamental components of landscape patterns" or "redundancy and parsimony in landscape metrics"-->
While recent years have seen the development of a large body of landscape metrics, the growing number of available metrics poses a major challenge in terms of their correlation and redundancy. <!-- [@Frazier2017LandscapeMetrics]. -->
To address these limitations, researchers have used techniques such as factor analysis, classification trees [@Breiman2017] and principal component analysis (PCA).
For instance, the seminal study of Riitters et al. [@Riitters1995] computed multiple metrics across a variety of landscapes and applied multivariate factor analysis to identify six main components of landscape pattern. <!-- namely average patch compaction, image texture, average patch shape, patch perimeter-area scaling, number of attribute classes and large-patch density-area scaling. -->
Another comprehensive work by Cushman et al. [@Cushman2008Parsimony] performed a PCA to isolate the structural components of a set of diverse landscapes, and extended the analysis to assess their strength (proportion of variance explained, which can be measured by the component eigenvalue), universality (how present they are accross landscapes) and consistency (how stable they are accross landscapes).
<!-- At the class level, they find seven "highly universal and consistent structure components" (page 700), namely edge contrast, patch shape complexity, aggregation, nearest neighbor distance, patch dispersion, large patch dominance and neighborhood similarity. At the landscape level, five components are identified, namely contagion/diversity (similar to image texture in Riitters et al. [@Riitters1995]), large patch dominance, interspersion/juxtaposition, edge contrast, patch shape variability, proximity and nearest neighbor distance. Overall, components are more consistent and universal at the landscape level than at the class level. -->
<!-- MB: I suppose we can ommit these:
...
- Additionally, multicollinearity in landscape metrics can be assessed using the Variance Inflation Factor (VIF) [@Plexida2014].
-->
<!-- More recently, the study of [@Nowosad2018] divided the entire Earth's land cover map into landscape blocks and performed PCA over a set of landscape metrics. The results suggest that there -->
More recently, Nowosad and Stepinski [@Nowosad2018] conducted PCA over a set of landscape blocks covering the entire terrestial landmass and resolved two main components of landscape configuration, namely complexity and aggregation (which together explain a 70% of the dataset variance).
The findings are in line with recent views [@Gustafson2019; @Riitters2019] suggesting that there are two fundamental components of landscape patterns, namely amount and adjacency - which are respectively connected to the complexity and aggregation components of Nowosad and Stepinski [@Nowosad2018].
<!-- Actually, some suggest that maybe only amount matters (Fahrig 2013; 2017). -->
<!-- MB: I wonder if we could already mention [@Nowosad2019InformationTheory] here since it presents the same two-component view of landscapes and links with the following section quite naturally -->
...
<!-- Software: Besides FRAGSTATS (to compute metrics), most studies use old-school software such as SAS and CANOCO4 (e.g., [@Cushman2008Parsimony]), STATISTICA (e.g., [@Uuemaa2011]), SPSS (e.g., [@Plexida2014]) -->
<!-- MB: should we ommit SAS and SPSS? -->
Given the computed landscape metrics, PCA and other dimension reduction techniques can be performed with stand-alone software such as SAS or SPSS, the R programming language or Python libraries such as statsmodels [@Seabold2010] or scikit-learn [@Pedregosa2011].
...
@Stepinski2022CurbYour and @Huckeba2023MultiscaleSpatial showed that the Shannon entropy is a general form of entropy, and other entropies, including the Boltzmann entropy, could be derived as specific applications of the Shannon entropy.
...
The *GeoStats.jl* [@Hoffimann2018GeoStatsjlHighperformance] and *GeoInterface.jl* [@JuliaGeoandcontributors2024GeoInterfaceJulia] software provide general spatial data operations in the Julia programming language.
