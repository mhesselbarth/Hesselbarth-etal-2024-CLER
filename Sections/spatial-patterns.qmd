...
@daleSpatialPatternAnalysis2000 defined them as the scale-dependent predictability of the physical arrangement of observations.
A more general definition by @grimmPatternOrientedModellingPopulation1996 describes patterns as a characteristic, clearly identifiable structure in nature itself or the data extracted from nature, indicating an underlying process that generates this pattern.
...
Thus, untangling the complex history of a landscape from its spatial patterns is a a golden grail of landscape ecology.

Here, our focus is on the computational approaches to spatial patterns.
Computational approaches to spatial patterns open many new possibilities compared to traditional methods based on visual interpretation.
...
Another approach is the "surface model" [@mcgarigalGradientConceptLandscape2005a], where the landscape is represented as continuous raster data with different values of some variable, such as elevation or temperature <!--@costanzaDescribingAnalyzingLandscape2019 for more refs-->.
...
Another noteworthy data type is vector data, representing point, line, and polygon features.
Here, the spatial patterns could be related to densities of points, orientation of lines, or shapes of polygons.
...
<!-- data sources??+SAR -->
Currently, the most common data sources in landscape ecology are remote sensing-based data, either in a direct form (satellite images) or in an indirect form (spectral indices, land cover maps).
...
<!-- refs?-->
Other examples are the increasing availability of high-resolution data (e.g., Sentinel-2) and the growing popularity of LiDAR and SAR data.
...
Many classification systems exists that are relevant on a local, regional, or global scale, such as the NLCD, the CORINE, or the FAO systems [@neddSynthesisLandUse2021].
Their definition of the categories could be different, and the same category could have different meanings in different systems.
<!-- ... -->
...
Moreover, the misclassification is not random, but it is often related to the land cover type, the region, and the time of the year [@liuFinerResolutionMappingGlobal2021].
...
Most methods and software tools in landscape ecology assume that the input data is in a Cartesian coordinate reference system, where the distance between two points is the Euclidean distance (most often in meters).
This assumption a significant impact, especially on measures related to distances and areas.
However, when the provided data is in a geographic coordinate reference system (with units in degrees), where the distance between two points is the great-circle distance, the results of the analysis could be incorrect.
At the same time, many projected coordinate reference systems (with units in, e.g., meters) exist, each with its own properties -- some distort areas, some distances, some angles, and some distort all of them.
Thus, the data may require reprojection before other calculations and the decision on the spatial reference system should be made based on the research question.
...
Entropy in landscape ecology is mainly used to quantify the complexity of the landscape (spatial heterogeneity) and less often unpredictability (temporal heterogeneity) and scale dependence (spatio-temporal heterogeneity) (@vrankenReviewUseEntropy2015a).
It is derived using various methods, mainly stemmed from the information theory and thermodynamics, including Shannon entropy (and its analogous forms, such as Simpson index<!-- Simpson index @simpsonMeasurementDiversity1949 -->), Boltzmann entropy, Renyi entropy, and more.

<!-- information (Shannon)/Shannon diversity -->
The Shannon diversity index (SHDI, @Shannon1948) it quantifies the richness and evenness of the categories in the landscape, with higher values indicating higher diversity (more different classes and more even distribution of them).
...
However, other measures from the information theory can be adapted to landscape ecology to quantify the spatial configuration and the total complexity of the landscape.
...
Original Shannon's entropy formula can also be modified to include the landscape's spatial configuration, e.g., by weights calculated from intra- and inter-class distances, as shown by @claramuntSpatiotemporalFormEntropy2012.
...
The overreaching goal of such studies is to link landscape ecology concepts to thermodynamic principles, mainly the second law of thermodynamics.
...
It is also worth noting that later, he refined and generalized this approach for calculations based on raster's surface model (based on mean slope) and point patterns (based on the average distance between points) [@cushmanGeneralizingBoltzmannConfigurational2021].
Subsequently, @Gao2017 proposed to use the Boltzmann entropy to quantify the complexity of a landscape surface in raster data by transforming the input grid into a series of landscape sufraces with different levels of detail (microstate) and calculating the Boltzmann entropy based on the total number of microstates that are able to generate the observed macrostate.
In later attempt, @zhaoCalculatingSpatialConfigurational2019 extended the original definition of Boltzmann entropy to incorporate the information about the adjacency of the same categories in the landscape mosaic by using the number of contiguous patches of the same category.
...
@stepinskiCurbYourEnthusiasm2022 and @huckebaMultiscaleSpatialEcology2023 showed that the Shannon entropy is a general form of entropy, and other entropies, including the Boltzmann entropy, could be derived as specific applications of the Shannon entropy.
...
Another recent developement is the use of Kullback-Leibler divergence (also known as relative entropy), which is a measure of the difference between two probability distributions, to describe patterns across scales [@huckebaMultiscaleSpatialEcology2023].

Importantly, the exisiting studies on entropy-based measures clearly show that the outcomes do not only depend on the various formulations of the entropy, but also on the underlining data representation [@zhaoCalculatingSpatialConfigurational2019].
The same entropy equation could provide different insights when applied just to the composition of the categories, co-occurrence matrix, or on the landscape transformed into a series of data with different levels of detail.
Thus, more work in this area is needed to understand the relations between the entropy measures and the landscape patterns they represent.
...
Thus, converting the polygons to raster data (*rasterization*) is a lossy process, in which information about the intricate details of the polygon shapes is lost.
Moreover, the rasterization process largely depends on the decided resolution of the raster outcome.

As mentioned, the initial computational approaches in landscape ecology were based on raster data.
However, the decision on the data model used was not theoretical but rather practical, as at the time, the most commonly used related software was raster-based, such as GRASS GIS.
...
Most landscape ecology studies are still based on raster data, probably due to the familarity with the techiniques, the availability of the data, and the availability of the software.
For example, most land cover maps (as LULC products are often related to underlying remote sensing products), including all on the global scale, are provided as rasters. 
At the same time, nowadays, vector data of land cover on regional or local scales is also available, and most geospatial software supports vector data.
...
@yaoVecLIFrameworkCalculating2022 showed that this is possible for most of the metrics, with only several exceptions (of the shape and aggregation group), whose parameters are related to the raster-format structure.
...
While landscape ecology studies have focussed on using raster data, the vector data model has been widely used in other fields, such as geography and urban studies.
...
Vector-based metrics are now popular, for example, in urban planning, as they allow to quantify the shapes of urban areas, e.g., @basaranerPerformanceShapeIndices2017a used 18 existing metrics and two new ones to characterize the complexity of building footprints.
<!-- more -->

The vector-based metrics also have their limitations.
The most important is the computational complexity, which makes the calculation of vector metrics slower than rasters ones, especially for large datasets.
Another technical issue is the need for the data to be topologically correct, which is not always the case, especially for data from different sources.
Vector-based metrics are also much less understood than raster-based ones, and their relations to landscape patterns still need to be explored.
...
The most basic operation, the comparison of spatial patterns, can be performed for just a pair of areas or the same area in two different moments.
It can also be extended straightforwardly by dividing the landscape into multiple smaller areas and comparing them at two moments in time.
Comparing spatial patterns is often used to analyze landscape dynamics, e.g., to detect changes in landscape structure over time [@duncanMonitoringSpatialPatterns2023].
Pattern-based search is a one-to-many operation in which the spatial signature of a query area is compared to the spatial signatures of all other areas.
