...
However, the decision on the data model used was not theoretical but rather practical, as at the time, the most commonly used software was raster-based, such as GRASS GIS.
...
For example, most land cover maps (as LULC products are often related to underlying remote sensing products), and all on the global scale, are provided as rasters. 
...
@yaoVecLIFrameworkCalculating2022 showed that this is possible for most of the metrics, with only several exceptions of metrics (of the shape and aggregation group), whose parameters are related to the raster-format structure.
