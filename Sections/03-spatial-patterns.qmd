...
The strength of landscape metrics is that they are easy to apply, communicate, and can be extracted straightforwardly from raster data based on remote sensing products [@Dramstad2009SpatialMetrics; @Lausch2015UnderstandingQuantifying].
...
Related to surface metrics, another recent approach to quantify the gradient surface model is based on frequencies and local adjacencies of continuous input pixel values [@Riitters2023MappingLandscape].
...
As hundreds of metrics have already been developed for raster data, the most straightforward approach may be to reimplement these same metrics for vector data.
