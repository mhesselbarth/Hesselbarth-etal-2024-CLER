...
\textcolor[HTML]{007895}{Reply: This is a very helpful comment about the general structure of the manuscript and we tried to address it as much as possible. This included several points. First, we agree that a general section on data is useful for readers. Thus, we added such a section at the beginning of the manuscript. Second, we modified especially the first paragraphs of most (sub-)sections in the "Spatial patterns" section to improve the logical flow of the manuscript. Third, we rearranged and modified the "Connectivity" section as well. We removed the "Structural connectivity" sub-section by including the key statements into the general paragraph about connectivity.}

**Other comments:**

L54. Data-driven and model-driven

\textcolor[HTML]{007895}{Reply: We added the term "model-driven" as suggested.}
...
\textcolor[HTML]{007895}{Reply: Indeed, such a table is very helpful. Thus, we removed all paragraphs that only list software from the specific sections and now list all software in a central table (Tab. 1).}
...
\textcolor[HTML]{007895}{Reply: We agree that landscape metrics can be extracted straightforwardly from raster data, and we are adding this as an advantage related to them.}

L92. Missing subsection title?

\textcolor[HTML]{007895}{Reply: Indeed, we forgot to add a sub-section title which is added now.}

L104. "of the selected entropy [measure]"?

\textcolor[HTML]{007895}{Reply: We replaced "entropy" with "measure".}

L119. "the edge length" -> Could you specify a bit?

\textcolor[HTML]{007895}{Reply: We added a brief definition of edge length in this context.}

L143. I guess that such metrics are also very scale dependent

\textcolor[HTML]{007895}{Reply: Similar to landscape metrics, also surface metrics are scale-dependent because both are designed to quantify landscape heterogeneity - which is inherently scale-dependent. We added this explanation including an according literature reference.}
...
\textcolor[HTML]{007895}{Reply: We removed the term "query" from the sentence because we agree that it is a source of confusion. However, we believe that the sentence using only "areas" instead is clear enough for readers to follow.}
...
\textcolor[HTML]{007895}{Reply: We agree that the whole section was confusing for readers unfamiliar with the topic. Thus, we reworded it to make it easier to follow by removing a lot of the database jargon.}

L175. This subsection is missing in the summary Figure 1.

\textcolor[HTML]{e93c26}{Reply: Thank you for pointing this out. We added it to the figure.}
...
\textcolor[HTML]{007895}{Reply: We added "flow" as suggested.}

L200. "movement [and dispersal] data"

\textcolor[HTML]{007895}{Reply: We added "dispersal" as suggested.}
...
\textcolor[HTML]{007895}{Reply: We added the specification that movement also includes dispersal of plants. Furthermore, we believe that this aspect is now also highlighted due to the other suggestions of the reviewer that we implemented in this section. Generally, we added this each time movement is mentioned in the section.}

L219. "to traverse [or disperse through] that cell"

\textcolor[HTML]{007895}{Reply: We added "disperse" as suggested.}

L221-222. Please explain what "edge artifacts" are. I could not find any explanation in the cited reference.

\textcolor[HTML]{007895}{Reply: We are unsure why the point about edge artifacts was added to the manuscript. Thus, we completely removed it here since we think it adds more confusion than helping the readers.}
...
\textcolor[HTML]{007895}{Reply: We agree that the original statement about underlying assumptions was unclear and reworded the sentence.}

L255-257. Edges can be weighted or unweighted. They also can be directed or undirected.

\textcolor[HTML]{007895}{Reply: We added this specification as suggested.}
...
\textcolor[HTML]{007895}{Reply: Thank you very much for adding this insightful remark about the references. We reworded the sentence to ensure the remarks are reflected in the manuscript.}
...
\textcolor[HTML]{007895}{Reply: We added that digital twins benefit from the increasing rate of data generation in ecology.}

L388. Spatial conservation planning -> spatial planning

\textcolor[HTML]{007895}{Reply: We renamed the section as suggested.}

L405. Reference 204 relies on constraint programming, not linear programming

\textcolor[HTML]{007895}{Reply: We reworded the sentence being more specific about the corresponding references.}
...
\textcolor[HTML]{007895}{Reply: Thank you for pointing out this detail. We added this as another potentail drawback of the method.}
...
\textcolor[HTML]{007895}{Reply: We reworded the sentence stating that there are differences between solvers and that the optimal solver depends on the specific problem. However, the best solver could potentially not be accessible due to software licenses.}
