...
\subsection{Landscapes generators}\label{landscapes-generators}

Landscape generators are typically used as null hypotheses, baselines,
or scenarios in landscape ecology analyses because they allow to control
certain aspects of the landscape (Li et al. 2004; Wang and Malanson
2008) and are generally simpler as ecological simulations models (Hess
et al. 2020). Landscape generators can be classified into two major
categories, namely pattern-based and process-based approaches (Pe'er et
al. 2013).

Pattern-based landscape generators simulate landscape patterns without
assuming any underlying abiotic or biotic processes (i.e., neutral
landscape model; Gardner et al. 1987). The earliest neutral models are
based on percolation theory and assign LULC types randomly to cells in
the landscape(Gardner et al. 1987) or include hierarchical models that
consider different spatial scales while assigning cell values (O'Neill
et al. 1992). Landscapes characterized by continuous values can be
simulated by fractal models, such as Brownian motion (Palmer 1992).
Borrowing from computer graphics and being able to simulate hierarchical
landscapes, more recent neutral landscape models make use of spectral
synthesis (e.g., Perlin noise; Etherington 2022) or Binary space
partitioning (Etherington et al. 2022). Neutral landscape models are
also able to simulate landscapes dominated by anthropogenic activities,
e.g., based on least-cost networks (Etherington et al. 2024). In order
to ensure realistic landscapes, several approaches exist that use real
landscapes (Inkoom et al. 2017) or are based on target values
optimization (Van Strien et al. 2016; Justeau-Allaire et al. 2022) to
simulate neutral landscapes.

Contrastingly, processes-based landscape generators explicitly include
hypothesized pattern creating processes (Langhammer et al. 2019;
Salecker et al. 2019).
